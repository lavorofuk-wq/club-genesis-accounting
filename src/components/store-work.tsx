...
function DailyWorkflow({ data, user, busy, run, initial, onFinished }: Props & { initial: DailyClosing | null; onFinished: () => void }) {
  ...
  const cash = pos ? calculateCash({ sales: pos.sales, cashFloat: data.cashFloat, expenses: expenseTotal, regularDailyPayments, trialDailyPayments, staffDailyPayments, driverDailyPayments, dispatchCastPayment, dispatchStaffPayment, dispatchFee, actualClosingCash: actualCash }) : null;
  ...
  return <Card title={initial ? `${initial.businessDate} 再編集` : "当日営業データ作成"} description="POS JSONの照合から現金実在高まで順番に確認します。" action={initial ? <button className="button secondary" onClick={onFinished}>再編集を終了</button> : null}>
    ...
    {stage === "details" && pos && <div className="stack section-pad">
      ...
      <h3>送迎ドライバー・日払い</h3><div className="check-grid">{data.drivers.filter((row) => row.status === "active").map((row) => { const selected = driverWork.some((item) => item.driverId === row.id); return <label className="select-card" key={row.id}><input type="checkbox" checked={selected} onChange={(e) => setDriverWork(e.target.checked ? [...driverWork.filter((item) => item.driverId !== row.id), { driverId: row.id, name: row.name, dailyRate: row.dailyRate, dailyPayment: 0 }] : driverWork.filter((item) => item.driverId !== row.id))} /><span>{row.name}<small>日給 {yen.format(row.dailyRate)}</small></span></label>; })}</div><Table headers={["ドライバー", "日給", "日払い"]}>{driverWork.map((row) => <tr key={row.driverId}><td>{row.name}</td><td>{yen.format(row.dailyRate)}</td><td><MoneyInput value={row.dailyPayment} onChange={(value) => setDriverWork((rows) => rows.map((item) => item.driverId === row.driverId ? { ...item, dailyPayment: value } : item))} /></td></tr>)}</Table>
