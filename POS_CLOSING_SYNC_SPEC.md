...
有償のシャンパン・ワイン／キープボトルは、本指名卓またはフリー卓の場内延長開始後だけ
ボトルバック対象を設定する。フリー時のボトルは `backTargetCastIds` と
`backTargetCastNames` を空配列とし、`backType` と `backAllocation` は設定しない。
対象が1名なら `backAllocation: "single"`、複数名なら `backAllocation: "equal"` とする。
複数対象の売上・酒代原価・ボトルバックはGMSが人数で均等分配する。

バック対象があるボトルだけ酒代原価マスターを種別・名称・販売金額で照合し、不一致時は取込を停止する。
店舗担当者が「今回のみの特別原価」を明示入力した場合だけ続行できる。

## 紹介者報酬の売上基準

- 売上10%は本指名売上の10%とし、場内延長売上は含めない。
- 酒代原価引き売上10%は「本指名売上－本指名時に計上された酒代原価」の10%とする。
- 場内延長時に計上された酒代原価は、紹介者報酬の原価引き売上から控除しない。
